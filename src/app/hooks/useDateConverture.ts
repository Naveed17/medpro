...
function useDateConverture(min: number, max: number) {
  ...
  const convertDate = useRef<any[]>([]); // eslint-disable-line react-hooks/exhaustive-deps
  let minutes: number[] = [];
  let diff = 1;
  convertDate.current = [];
  useEffect(() => {
    ...
    minutes.map((minute: number) => {
      const dates = Object.values(
        (moment.duration(minute, "minutes") as any)._data
      );
      ...
      convertDate.current = [
        ...convertDate.current,
        {
          ...
          unity:
            dates[index] === 1
              ? unities[index].singular
              : unities[index].plural,
        },
      ];
    });
