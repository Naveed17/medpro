...
function AgendaTimeLine() {
    ...
    return (
        <RootStyled spacing={1}>
            <Stack borderRadius={1.3} bgcolor={theme => theme.palette.background.default} direction='row' alignItems='center' justifyContent='space-between'>
                <IconButton disableRipple size='small' onClick={handleDecreesDays}>
                    <KeyboardArrowLeftIcon fontSize='small' color='primary' />
                </IconButton>
                <Typography fontWeight={600} color="primary">{today ? t("today") : startDate.toDateString()}</Typography>
