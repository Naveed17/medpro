import {CardContent, Link, Theme, Stack, Typography, IconButton} from '@mui/material'
                ...
                <Stack spacing={1}>
                        ...
                        <Stack spacing={1} direction="row" alignItems='center'>
                            ...
                            {data.insurance ?
                                <Stack direction='row' alignItems="center">
                                    ...
                                    <Image src={`/static/img/${data.insurance.img}.png`} width={20} height={20}
                                           alt={data.insurance.name}/>
                                </Stack> :
                                <Typography>--</Typography>
                            }
                        </Stack>
                    ...
                    <Stack spacing={4} direction="row" alignItems='center' className="insurrence">
                        <Stack spacing={1} direction="row" alignItems='center'>
                            {data.billing_status ?
                                <Label className="label" variant="ghost"
                                       color={data.billing_status === "yes" ? "success" : 'error'}>{t('table.' + data.billing_status)}</Label>
                                : <Typography>--</Typography>
                            }
                        </Stack>
                        {data.pending ? <Stack direction='row' spacing={2} alignItems="center">
                            ...
                            <Typography
                                color={(data.amount > 0 && 'success.main' || data.amount < 0 && 'error.main') || 'text.primary'}
                                fontWeight={700}>{data.amount}</Typography>
