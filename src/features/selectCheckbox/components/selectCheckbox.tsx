...
import {
  selectCheckboxActionSelector,
  onSelectCheckbox,
} from "@features/selectCheckbox";
function SelectCheckbox({ ...props }) {
  const { row, isSmall } = props;
  const dispatch = useAppDispatch();
  const {
    tableState: { rowsSelected },
  } = useAppSelector(tableActionSelector);
  const {
    selectCheckboxState: { selectedCheckbox },
  } = useAppSelector(selectCheckboxActionSelector);
  ...
  const handleCheckItem = (isItemSelected: boolean, row: PatientModel) => {
    if (isItemSelected) {
      ...
      dispatch(
        setSelectedRows(
          rowsSelected.filter((item: any) => item.uuid !== row.uuid)
        )
      );
    }
  };
