            ...
            <Box maxWidth={'100%'} mx="auto" id={"ctn"}>
                {loading ?
                    ...
                    <List sx={{width: '100%'}}>
                        ...
                        {state && antecedents.filter((item: AntecedentsTypeModel) => item.name.toLowerCase().includes(value.toLowerCase())).filter(item => state.find(s => s.uuid === item.uuid)).map((list: any, idx: number) =>
                        ...
                        {state && antecedents.filter((item: AntecedentsTypeModel) => item.name.toLowerCase().includes(value.toLowerCase())).filter(item => state.find(s => s.uuid === item.uuid) == null).map((list: any, idx: number) =>
                            <Stack key={idx} spacing={0}>
                                <ListItem>
                                    <FormGroup
                                        className={state?.find(inf => inf.uuid == list.uuid) !== undefined ? "selected-ant" : ""}
