...
function QualificationDialog(info: any) {
  ...
  }, [dispatch, items]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <>
      <Stack spacing={2} className="top-sec">
        ...
        <BasicAlert icon="ic-danger" data={t("profil.alertQ")} color="warning">
          .
        </BasicAlert>
        {files.length > 0 ? (
          <Stack spacing={2}>
            ...
            <List>
              {items.map((item) => (
                <SortableItem key={item.uuid} id={item.uuid}>
                  <QualifactionsProfessional item={item}>
                    .
