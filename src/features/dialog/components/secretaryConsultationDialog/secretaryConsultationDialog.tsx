...
function SecretaryConsultationDialog({ ...props }) {
  ...
  const { trigger: triggerAppointmentEdit } =
    useRequestQueryMutation("appointment/edit");
            ...
            {total - getTransactionAmountPayed() !== 0 ? (
              <Stack direction={"row"} alignItems={"center"}>
                ...
                <Label
                  variant="filled"
                  color={
                    getTransactionAmountPayed() === 0 ? "success" : "warning"
                  }
                ...
                {demo && (
                  ...
                  >
                    <IconUrl color={"white"} path="ic-fees" />{" "}
                    {!isMobile && (
        ...
        <Grid item md={6} sm={12} xs={12}>
          ...
          >
            ...
            >
              {changes.map(
                (
                  item: { checked: boolean; icon: string; name: string },
                  idx: number
                ) => (
                  <Badge
                    key={"feat" + idx}
