...
function PlacesDetail() {
    ...
    onSubmit: async (values, { setErrors, setSubmitting }) => {
      ...
      if (data) {
        ...
        url =
          ...
          medical_entity.uuid +
          "/locations/" +
          (data as HttpResponse).data.uuid +
          "/" +
          router.locale;
      } else {
        method = "POST";
  ...
  const { data: httpStateResponse } = useRequest({
    method: "GET",
    url:
      "/api/public/places/countries/" +
      medical_entity.country.uuid +
      "/state/" +
      router.locale,
    headers: { Authorization: `Bearer ${session?.accessToken}` },
  });
      ...
      trigger(
        {
          method: "GET",
          url:
            "/api/public/places/state/" +
            adr.address.state.uuid +
            "/cities/" +
            router.locale,
          headers: {
            ContentType: "application/x-www-form-urlencoded",
