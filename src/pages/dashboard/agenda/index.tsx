...
function Agenda() {
  ...
  appointments = (
    eventCond?.hasOwnProperty("list") ? eventCond.list : eventCond
  ) as AppointmentModel[];
  appointments?.map((appointment) => {
    events.push({
      start: moment(
        appointment.dayDate + " " + appointment.startTime,
        "DD-MM-YYYY hh:mm"
      ).toDate(),
      time: moment(
        appointment.dayDate + " " + appointment.startTime,
        ...
        .add(appointment.consultationReason.duration, "minutes")
        .toDate(),
  ...
  const groups: any = events.reduce((groups: any, data: any) => {
    const date = moment(data.time, "ddd MMM DD YYYY HH:mm:ss").format(
      "DD-MM-YYYY"
    );
    if (!groups[date]) {
      groups[date] = [];
  ...
  return (
    <>
      ...
      <Box>
        ...
        <MobileContainer>
          {sortedData?.map((row, index) => (
            <Container key={index}>
              ...
              >
                {moment(row.date, "DD-MM-YYYY").isSame(
                  moment(new Date(), "DD-MM-YYYY")
                ) ? (
                  "Today"
                ) : moment(row.date, "DD-MM-YYYY").isSame(
                    moment(new Date(), "DD-MM-YYYY").add(1, "days")
                  ) ? (
                  "Tomorrow"
                ) : (
                  <>
        ...
        >
          {event && <AppointmentDetail translate={t} data={event} />}
        </Drawer>
        <Drawer
          anchor={"right"}
          open={openAddDrawer}
